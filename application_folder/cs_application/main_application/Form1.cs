        ...
        */
        private void ProcessFrameData()
        {
            ...
            try
            {
                ...
                utf8data = serialPort1.ReadExisting();
                ...
                dynamic utf8result = function(utf8data);
        ...
        /* 
            ...
            frame type ={0,..,0x7F}
            data length ={0,..,0x7F}
            Data ={0,..,..,n} n <=0x7F-1 //Not more than 128 chars
            StopByte = 0xFF

        */

        //Method is not used yet
        private byte[] HeaderProcessor(byte[] header)
        {
            
            byte destination_address = header[0];
            byte sender_address = header[1];
            byte frame_type = header[2];
            byte data_length = header[3];
            
            return header;
        }

        //Method is not used yet
        private string decodeReceivedData(string ReceivedData)
        {
            return null ;
        }
        //Method is not used yet
        private string EncodeDataToSend(string DataToSend)
        {
            return null;
        }
        ...
        private void serialPort1_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            //Check frame beginning
            /*int StartByte = serialPort1.ReadByte();
           
            if (StartByte == 0x7F)
            {
                Console.WriteLine("Start of frame detected");
            }*/

        }

        //Method is not used yet
        private void FormNewFrame()
        {
            label4.Text = "Sending";
            ...
            try
            {
                ...
                dynamic result = function(utf8message);
                var encoded_message = result;

                asciimessage = Convert.ToString(encoded_message);

                serialPort1.Encoding = Encoding.ASCII;

                serialPort1.Write(asciimessage);
            }
            catch (InvalidOperationException ex)
            {
        ...
        //Make New Frame To Send, Data is filled by encoded 7-bit ascii
